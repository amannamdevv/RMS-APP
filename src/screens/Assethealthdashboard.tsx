/**
 * AssetHealthScreen.tsx
 *
 * Backend response structure (from views.py):
 *
 * OVERVIEW  → res.overview.{ total_critical, total_warning, total_healthy,
 *              total_overloaded, total_sites, health_percentage, sites_with_issues }
 *
 * BATTERY   → res.summary.{ total_sites, health_categories.{ good, average,
 *              needs_replacement, critical, data_insufficient } }
 *             res.categories.{ health_good[], health_average[],
 *              health_needs_replacement[], health_critical[], health_data_insufficient[] }
 *             Each site: health_status, current_voltage, current_current, battery_ah,
 *              configuration, battery_type, make, battery_backup,
 *              backup_verification_status, backup_verification_message,
 *              longest_session_minutes, avg_session_minutes, total_sessions_analyzed,
 *              battery_serial_no, battery_model_no, battery_voltage_v,
 *              parallel_config, battery_year
 *
 * DG        → res.summary.{ total_sites, total_dg_sites, no_dg_sites,
 *              dg_categories.{ dg_high_risk, dg_at_risk, dg_loading_above_90,
 *              no_dg, healthy, stopped } }
 *             res.categories.all_sites[]
 *             Each site: status, today_dg_hours, loading_percent, dg_make,
 *              dg_model, rated_capacity_kva, dg_serial_no, dg_controller,
 *              dg_software, dg_phase, amf_units
 *
 * RECTIFIER → res.summary.rectifier_categories.{ not_n_plus_1, has_faults,
 *              insufficient_capacity, healthy, no_rectifier }
 *             res.categories.{ not_n_plus_1[], has_faults[], insufficient_capacity[],
 *              healthy[], no_rectifier[] }
 *             Each site: status, is_n_plus_1, sufficient_capacity, current_load,
 *              rectifier_capacity_amp, total_rectifiers, working_rectifiers,
 *              faulty_rectifiers
 *
 * SOLAR     → res.summary.{ total_solar_sites, total_sites,
 *              solar_categories.{ performance_good, performance_average,
 *              performance_poor, not_installed } }
 *             res.categories.{ performance_good[], performance_average[],
 *              performance_poor[], not_installed[] }
 *             Each site: performance_status, actual_cuf, expected_cuf,
 *              solar_capacity_kw, panel_count, mppt_count, faulty_mppt_count
 *
 * DG BATTERY→ res.summary.dg_battery_categories.{ battery_missing, battery_critical,
 *              battery_warning, battery_good }
 *             res.categories.{ battery_missing[], battery_critical[],
 *              battery_warning[], battery_good[] }
 *             Each site: health_status, battery_voltage, battery_present,
 *              dg_installed, battery_make, last_checked
 *
 * LIGHTNING → res.summary.la_categories.{ la_missing, la_blown, la_needs_check,
 *              la_functional, inspection_overdue }
 *             res.categories.la_needs_check[]
 *             Each site: status, la_present, la_count, last_inspection_date, remarks
 */
...
// ─────────────────────────────────────────────────────────────
// BATTERY SCREEN
// backend: res.summary.health_categories.{ good, average, needs_replacement, critical, data_insufficient }
//          res.categories.{ health_good[], health_average[], health_needs_replacement[],
//                           health_critical[], health_data_insufficient[] }
// ─────────────────────────────────────────────────────────────
function BatteryScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const cats = data?.categories || {};

    // Combine all sites — critical first
    const sites: any[] = [
        ...
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="Battery Health" total={sum?.total_sites} />
            ...
            ListEmptyComponent={<Empty msg="No battery data" />}
        />
    );
}
...
// ─────────────────────────────────────────────────────────────
// DG SCREEN
// backend: res.summary.{ total_dg_sites, no_dg_sites, dg_categories.{...} }
//          res.categories.all_sites[]
// ─────────────────────────────────────────────────────────────
function DGScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const sites: any[] = data?.categories?.all_sites || [];
    ...
    return (
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="DG Health" total={sum?.total_sites} />
                    <SummaryRow items={summItems} />
                </View>
            }
            renderItem={({ item }) => (
            ...
            ListEmptyComponent={<Empty msg="No DG data" />}
        />
    );
}
...
// ─────────────────────────────────────────────────────────────
// RECTIFIER SCREEN
// backend: res.summary.rectifier_categories.{ not_n_plus_1, has_faults,
//           insufficient_capacity, healthy, no_rectifier }
//          res.categories.{ not_n_plus_1[], has_faults[], insufficient_capacity[],
//           healthy[], no_rectifier[] }
// ─────────────────────────────────────────────────────────────
function RectifierScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const cats = data?.categories || {};
    ...
    return (
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="Rectifier Health" total={sum?.total_sites} />
                    <SummaryRow items={summItems} />
                </View>
            }
            renderItem={({ item }) => (
            ...
            ListEmptyComponent={<Empty msg="No rectifier data" />}
        />
    );
}
...
// ─────────────────────────────────────────────────────────────
// SOLAR SCREEN
// backend: res.summary.{ total_solar_sites, total_sites,
//           solar_categories.{ performance_good, performance_average,
//           performance_poor, not_installed } }
//          res.categories.{ performance_good[], performance_average[],
//           performance_poor[], not_installed[] }
// ─────────────────────────────────────────────────────────────
function SolarScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const cats = data?.categories || {};
    ...
    return (
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="Solar Health" total={sum?.total_sites} />
                    <SummaryRow items={summItems} />
                </View>
            }
            renderItem={({ item }) => (
            ...
            ListEmptyComponent={<Empty msg="No solar data" />}
        />
    );
}
...
// ─────────────────────────────────────────────────────────────
// DG BATTERY SCREEN
// backend: res.summary.dg_battery_categories.{ battery_missing, battery_critical,
//           battery_warning, battery_good }
//          res.categories.{ battery_missing[], battery_critical[],
//           battery_warning[], battery_good[] }
// ─────────────────────────────────────────────────────────────
function DGBatteryScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const cats = data?.categories || {};
    ...
    return (
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="DG Battery Health" total={sum?.total_sites} />
                    <SummaryRow items={summItems} />
                </View>
            }
            renderItem={({ item }) => (
            ...
            ListEmptyComponent={<Empty msg="No DG battery data" />}
        />
    );
}

// ─────────────────────────────────────────────────────────────
// LIGHTNING ARRESTER SCREEN
// backend: res.summary.la_categories.{ la_missing, la_blown, la_needs_check,
//           la_functional, inspection_overdue }
//          res.categories.la_needs_check[]
// ─────────────────────────────────────────────────────────────
function LightningScreen({ data, refreshing, onRefresh }: { data: any; refreshing: boolean; onRefresh: () => void }) {
    const sum = data?.summary;
    const sites: any[] = data?.categories?.la_needs_check || [];
    ...
    return (
        <FlatList
            ...
            ListHeaderComponent={
                <View style={{ marginBottom: 14 }}>
                    <SectionHeader label="Lightning Arrester" total={sum?.total_sites} />
                    <SummaryRow items={summItems} />
                </View>
            }
            renderItem={({ item }) => (
            ...
            ListEmptyComponent={<Empty msg="No lightning arrester data" />}
        />
    );
}
...
export default function AssetHealthScreen({ navigation, route }: any) {
    ...
    // Sidebar se alag tab pe navigate karo
    useEffect(() => {
    ...
    // Lazy load — fetch only once unless refreshed
    useEffect(() => {
    ...
    const onRefresh = () => {
        setRefreshing(true);
        // Force re-fetch by clearing current tab data
    ...
    function renderScreen() {
        ...
        switch (activeTab) {
            case 'battery': return <BatteryScreen {...commonProps} />;
            case 'dg': return <DGScreen {...commonProps} />;
            case 'rectifier': return <RectifierScreen {...commonProps} />;
            case 'solar': return <SolarScreen {...commonProps} />;
            case 'dg_battery': return <DGBatteryScreen {...commonProps} />;
            case 'lightning': return <LightningScreen {...commonProps} />;
            default: return null;
        }
    }
    ...
    return (
        <SafeAreaView style={MS.container}>
            ...
            {/* Header */}
            <LinearGradient colors={['#1e3c72', '#2a5298']} style={MS.header}>
                <TouchableOpacity onPress={() => setSidebarVisible(true)} style={MS.hBtn}>
                    <Icon name="menu" size={22} color="#fff" />
                </TouchableOpacity>
                <View style={{ alignItems: 'center' }}>
                    <Text style={MS.hTitle}>ASSET HEALTH</Text>
                    <Text style={MS.hSub}>{currTab?.label}</Text>
                </View>
                <TouchableOpacity onPress={onRefresh} style={MS.hBtn}>
                    <Icon name="refresh-cw" size={18} color={refreshing ? '#10b981' : '#fff'} />
                </TouchableOpacity>
            </LinearGradient>
...
const MS = StyleSheet.create({
    ...
    hBtn: { padding: 6 },
    hTitle: { color: '#fff', fontSize: 15, fontWeight: '800', letterSpacing: 1.2 },
