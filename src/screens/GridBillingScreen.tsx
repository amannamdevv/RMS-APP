 ...
 *   states_list: [{state_id, state_name}],
 *   districts_list: [{dist_id, district_name}],
 *   sites_list: [{imei, site_id, site_name, state_name, district_name}],
 *   kpis: { ac_uptime, phase_missing_incidents, low_voltage_alerts },
 *   overview: {
 *     voltage_trend:   { labels, values },
 *     tod_consumption: { labels, values }
 *   },
 *   quality_of_supply: {
 *     site_data: [{ site_id, site_name, r_phase_missing, y_phase_missing,
 *                   b_phase_missing, low_voltage_count, ac_uptime }],
 *     ryb_voltage_trend: { labels, r_phase, y_phase, b_phase },
 *     recent_alerts: [{ site_id, site_name, alert_type, timestamp }]
 *   },
 *   tod_monitoring: {
 *     consumption_by_site: { site_name: { slot: value } },
 *     time_slots,
 *     abnormal_alerts: {
 *       spike_alerts:    [{ site_id, site_name, period_avg, today_avg, deviation_pct, direction }],
 *       offhours_alerts: [{ site_id, site_name, offhours_avg, records, reason }],
 *       weekly_alerts:   [{ site_id, site_name, this_week_avg, last_week_avg, deviation_pct, direction }]
 *     }
 *   },
 *   technology_distribution: { labels, values }
 * }
 */
...
import {
    ...
    StatusBar, FlatList, TextInput, Modal,
} from 'react-native';
import { api } from '../api';
...
function MiniBar({ values, labels, colors }: { values: number[]; labels: string[]; colors: string[] }) {
            ...
            {values.map((v, i) => (
                <View key={i} style={{ marginBottom: 8 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 3 }}>
                        <Text style={{ fontSize: 10, color: '#64748b', fontWeight: '600' }}>{labels[i]}</Text>
                        <Text style={{ fontSize: 10, color: colors[i % colors.length], fontWeight: '800' }}>{fmt(v)}</Text>
                    </View>
                    <View style={{ height: 8, backgroundColor: '#f1f5f9', borderRadius: 4 }}>
                        <View style={{ height: 8, width: `${Math.min((v / max) * 100, 100)}%`, backgroundColor: colors[i % colors.length], borderRadius: 4 }} />
                    </View>
                </View>
            ))}
        </View>
...
function MiniLine({ values, labels, color }: { values: number[]; labels: string[]; color: string }) {
    const max = Math.max(...values, 1);
    ...
    return (
        <View style={{ marginTop: 8 }}>
            <View style={{ height: H, position: 'relative', marginBottom: 4 }}>
                {points.map((p, i) => (
                    <View key={i} style={{
                        position: 'absolute',
                        left: p.x - 5,
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                {labels.map((l, i) => (
                    <Text key={i} style={{ fontSize: 9, color: '#94a3b8', fontWeight: '600', textAlign: 'center', flex: 1 }}>{l}</Text>
                ))}
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 2 }}>
                {values.map((v, i) => (
                    <Text key={i} style={{ fontSize: 10, color: color, fontWeight: '800', textAlign: 'center', flex: 1 }}>{fmt(v)}</Text>
                ))}
            </View>
        </View>
    );
        ...
        <View style={[ACS.card, { backgroundColor: bg, borderLeftColor: color }]}>
            ...
            <View style={[ACS.badge, { backgroundColor: color }]}>
                <Text style={ACS.badgeTxt}>
                    {type === 'offhours' ? '⚡ OFF-HRS' : `${isSpike ? '▲' : '▼'} ${Math.abs(item.deviation_pct)}%`}
                </Text>
            </View>
        </View>
    );
    ...
    return (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 14, paddingBottom: 30 }}>
            ...
            <SectionCard title="Avg Voltage Trend (6-hr slots)">
                <MiniLine values={vt.values} labels={vt.labels} color="#5da3fa" />
            </SectionCard>
...
function QualityTab({ data }: { data: any }) {
    ...
    return (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 14, paddingBottom: 30 }}>
                        ...
                        {['R Phase', 'Y Phase', 'B Phase'].map((label, idx) => {
                            ...
                            return (
                                <View key={label} style={{ marginBottom: 10 }}>
                                    ...
                                    <MiniLine values={vals || []} labels={ryb.labels} color={colors[idx]} />
                                </View>
                            );
                        })}
                    </View>
            ...
            {/* Phase Missing Table */}
            <SectionCard title={`Sites with Phase Missing (${siteData.length})`}>
                {siteData.length === 0 ? (
                    <View style={{ alignItems: 'center', padding: 20 }}>
                        <Icon name="check-circle" size={24} color="#10b981" />
                        <Text style={{ color: '#10b981', fontWeight: '700', marginTop: 6 }}>No phase missing incidents</Text>
                    </View>
                ) : (
                    siteData.map((s: any, i: number) => (
            ...
            <SectionCard title="Recent Phase Missing Alerts">
                {alerts.length === 0 ? (
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        ...
                        <Text style={{ color: '#10b981', fontWeight: '600' }}>No recent alerts</Text>
                    </View>
                ) : (
...
function TODTab({ data }: { data: any }) {
    ...
    const siteNames = Object.keys(bySite).slice(0, 5);

    // Weekly chart data from weekly_alerts
    const wa = abn.weekly_alerts || [];
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 14, paddingBottom: 30 }}>
                    ...
                    {siteNames.map((name, idx) => {
                        ...
                        return (
                            <View key={name} style={{ marginBottom: 12 }}>
                                ...
                                <MiniBar values={vals} labels={slots} colors={[colors[idx % colors.length]]} />
                            </View>
                        );
                    })}
                </SectionCard>
            ...
            {wa.length > 0 && (
                <SectionCard title="This Week vs Last Week">
                    {wa.slice(0, 5).map((w: any, i: number) => (
                        <View key={i} style={{ marginBottom: 10 }}>
                            <Text style={{ fontSize: 10, fontWeight: '700', color: '#0f172a', marginBottom: 4 }}>{w.site_id}</Text>
                            <View style={{ flexDirection: 'row', gap: 8 }}>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 8, color: '#5da3fa', fontWeight: '700', marginBottom: 2 }}>This Week</Text>
                                    <View style={{ height: 6, backgroundColor: '#f1f5f9', borderRadius: 3 }}>
                                        <View style={{ height: 6, width: `${Math.min((w.this_week_avg / (Math.max(w.this_week_avg, w.last_week_avg) || 1)) * 100, 100)}%`, backgroundColor: '#5da3fa', borderRadius: 3 }} />
                                    </View>
                                    <Text style={{ fontSize: 9, color: '#5da3fa', fontWeight: '800', marginTop: 2 }}>{w.this_week_avg}</Text>
                                </View>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 8, color: '#1c3d5a', fontWeight: '700', marginBottom: 2 }}>Last Week</Text>
                                    <View style={{ height: 6, backgroundColor: '#f1f5f9', borderRadius: 3 }}>
                                        <View style={{ height: 6, width: `${Math.min((w.last_week_avg / (Math.max(w.this_week_avg, w.last_week_avg) || 1)) * 100, 100)}%`, backgroundColor: '#1c3d5a', borderRadius: 3 }} />
                                    </View>
                                    <Text style={{ fontSize: 9, color: '#1c3d5a', fontWeight: '800', marginTop: 2 }}>{w.last_week_avg}</Text>
                                </View>
                            </View>
                        </View>
                    ))}
            ...
            <View style={TODS.abnWrap}>
                ...
                {/* Spike/Drop */}
                {abnTab === 'spike' && (
                    abn.spike_alerts?.length ? abn.spike_alerts.map((a: any, i: number) => (
                        <AbnCard key={i} item={a} type="spike" />
                    )) : <Text style={TODS.noData}>✅ No period spikes / drops detected</Text>
                )}

                {/* Off-hours */}
                {abnTab === 'offhours' && (
                    abn.offhours_alerts?.length ? abn.offhours_alerts.map((a: any, i: number) => (
                        <AbnCard key={i} item={a} type="offhours" />
                    )) : <Text style={TODS.noData}>✅ No off-hours consumption detected</Text>
                )}

                {/* Weekly */}
                {abnTab === 'weekly' && (
                    abn.weekly_alerts?.length ? abn.weekly_alerts.map((a: any, i: number) => (
                        <AbnCard key={i} item={a} type="weekly" />
                    )) : <Text style={TODS.noData}>✅ No weekly deviations {'>'} 15% detected</Text>
                )}
            </View>
        </ScrollView>
    );
...
const TODS = StyleSheet.create({
    ...
    noData: { fontSize: 12, color: '#10b981', fontWeight: '600', padding: 16, textAlign: 'center' },
});
                ...
                <View style={FDS.drawer}>
                    <View style={FDS.header}>
                        ...
                        <TouchableOpacity onPress={onClose}>
                            <Icon name="x" size={22} color="#1e293b" />
                        </TouchableOpacity>
                    </View>
...
export default function GridBillingScreen({ navigation }: any) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <StatusBar barStyle="light-content" backgroundColor="#1c3d5a" />

            {/* Header */}
            <LinearGradient colors={['#1c3d5a', '#2c5282']} style={styles.header}>
                <TouchableOpacity onPress={() => setSidebarVisible(true)} style={styles.hBtn}>
                    <Icon name="menu" size={22} color="#fff" />
                </TouchableOpacity>
                <View style={{ alignItems: 'center', flex: 1 }}>
                    <Text style={styles.hTitle}>⚡ GRID POWER ANALYTICS</Text>
                    {!!filterSummary && (
                        <Text style={styles.hSub} numberOfLines={1}>{filterSummary}</Text>
                    )}
                </View>
            ...
            {loading ? (
                ...
                <View style={{ flex: 1, backgroundColor: '#edf2fb' }}>
                    {activeTab === 'Overview' && (
                        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#5da3fa']} />}>
...
const styles = StyleSheet.create({
    ...
    retryBtn: { marginTop: 16, backgroundColor: '#5da3fa', borderRadius: 10, paddingHorizontal: 24, paddingVertical: 10 },
