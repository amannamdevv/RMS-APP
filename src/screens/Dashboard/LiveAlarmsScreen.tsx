...
import {
    ...
    ActivityIndicator, SafeAreaView, RefreshControl, ScrollView, Alert
} from 'react-native';
import { api } from '../../api';
        ...
        return (
            ...
            >
                ...
                <View style={styles.infoRow}>
                    <View style={styles.infoBox}><Icon name="zap" size={12} color="#64748b" /><Text style={styles.infoText}> {item.site_running_status || 'N/A'}</Text></View>
                    <View style={styles.infoBox}><Icon name="clock" size={12} color="#64748b" /><Text style={styles.infoText}> {item.active_time_formatted || 'Active'}</Text></View>
                    <View style={[styles.infoBox, { borderRightWidth: 0 }]}><Icon name="activity" size={12} color="#1e3c72" /><Text style={[styles.infoText, { fontWeight: '700', color: '#1e3c72' }]}> {item.start_volt ? `${parseFloat(item.start_volt).toFixed(2)}V` : 'N/A'}</Text></View>
                </View>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.center}>
                            ...
                            <Text style={styles.emptyText}>No alarms found for selected filter</Text>
                        </View>
                    }
                />
            )}
...
const styles = StyleSheet.create({
    ...
    emptyText: { marginTop: 15, fontSize: 14, color: '#94a3b8', textAlign: 'center' }
});