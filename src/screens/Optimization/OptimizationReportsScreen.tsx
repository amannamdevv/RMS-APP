 ...
 *   'overview' | 'energy' | 'losses' | 'leakage' | 'voltage' |
 *   'power' | 'powerfactor' | 'sanctioned' | 'events'
 *
 * Live API (Revenue Leakage tab only):
 *   GET /api/revenue-data/?type=all|leakage|no_leakage|null
 *   Response: {
 *     status, total_sites, leakage_count, no_leakage_count, null_count,
 *     leakage_percent, no_leakage_percent, null_percent,
 *     avg_avg_load, avg_peak_load, avg_total_declared,
 *     rows: [{ site_id, imei, avg_load, peak_load, so_load,
 *              avg_load_leakage, peak_load_leakage, analysis_date }]
 *   }
 */
...
export default function OptimizationReportsScreen({ navigation, route }: any) {
    ...
    // Update tab from sidebar navigation
    useEffect(() => {
    ...
    // Filtered + paginated revenue rows
    const allRows: any[] = revData?.rows || [];
    const filtered = allRows.filter(r =>
        !search || r.site_id?.toLowerCase().includes(search.toLowerCase()) || (r.imei || '').includes(search)
    );
        ...
        <SafeAreaView style={S.container}>
            ...
            {activeTab === 'leakage' && (
                revLoading && !revData ? (
                    ...
                    <FlatList
                        ...
                        refreshControl={<RefreshControl refreshing={revRefresh}
                            onRefresh={() => { setRevRefresh(true); loadRevenue(filterType, true); }}
                            colors={[C.primary]} />}
                        ListHeaderComponent={revData ? (
                            <View style={{ paddingTop: 5 }}>
                                ...
                                <View style={S.card}>
                                    ...
                                    {/* Total sites */}
                                    <Text style={{ fontSize: 32, fontWeight: '800', color: C.primary, textAlign: 'center', marginBottom: 4 }}>
                                        {revData.total_sites}
                                    </Text>
                                    ...
                                    {[
                                        ...
                                        <View key={l.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                                            ...
                                            <Text style={{ fontSize: 12, fontWeight: '700', color: '#1e293b' }}>
                                                {l.pct}%  ({l.cnt})  <Text style={{ color: '#64748b', fontWeight: '500' }}>{l.label}</Text>
                                            </Text>
                                        </View>
                                    ))}
                                    {/* Metrics */}
                                ...
                                {/* Action Row MOVED BELOW Analytics */}
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 15 }}>
                                    <TouchableOpacity style={[S.filterBtn, { flex: 1 }]} onPress={() => setFilterModal(true)}>
                                        <Icon name="filter" size={14} color={C.primary} />
                                        <Text style={S.filterBtnTxt}>Filter: {FILTER_TYPES.find(f => f.v === filterType)?.l}</Text>
                        ...
                        ListFooterComponent={totalPages > 1 ? (
                            <View style={S.pagination}>
                                <TouchableOpacity style={[S.pageBtn, page === 1 && S.pageBtnDis]}
                                    onPress={() => { if (page > 1) setPage(1); }} disabled={page === 1}>
                                    <Text style={S.pageBtnTxt}>First</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={[S.pageBtn, page === 1 && S.pageBtnDis]}
                                    onPress={() => { if (page > 1) setPage(p => p - 1); }} disabled={page === 1}>
                                    <Icon name="chevron-left" size={14} color={C.primary} />
                                </TouchableOpacity>
                                <Text style={S.pageInfo}>{page} / {totalPages}</Text>
                                <TouchableOpacity style={[S.pageBtn, page === totalPages && S.pageBtnDis]}
                                    onPress={() => { if (page < totalPages) setPage(p => p + 1); }} disabled={page === totalPages}>
                                    <Icon name="chevron-right" size={14} color={C.primary} />
                                </TouchableOpacity>
                                <TouchableOpacity style={[S.pageBtn, page === totalPages && S.pageBtnDis]}
                                    onPress={() => { if (page < totalPages) setPage(totalPages); }} disabled={page === totalPages}>
                                    <Text style={S.pageBtnTxt}>Last</Text>
                                </TouchableOpacity>
                            </View>
                        ) : null}
                        ListEmptyComponent={!revLoading ? (
                            <View style={{ alignItems: 'center', paddingTop: 40 }}>
                                <Icon name="droplet" size={36} color="#cbd5e1" />
                                <Text style={{ color: '#94a3b8', fontSize: 13, marginTop: 12 }}>
                                    {search ? 'No sites match your search' : 'No revenue data available'}
                                </Text>
                            </View>
                        ) : null}
                    />
            ...
            {activeTab !== 'leakage' && (
                <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 14, paddingBottom: 30 }}
                    ...
                    {activeTab === 'losses' && (
                        <View style={S.card}>
                            <Text style={S.cardTitle}>High Conversion Loss Sites</Text>
                            <STable
                                headers={['Site', 'Loss (kWh)', 'Status']}
                                rows={[
                                    ['Delhi-Central-01', '312', '🔴 Critical'],
                                    ['Kolkata-East-12', '288', '🟡 Review'],
                                    ['Mumbai-West-19', '261', '🔵 Attention'],
                                    ['Chennai-South-05', '240', '🟢 Normal'],
                                ]}
                            />
                        </View>
                    )}
                    ...
                    {activeTab === 'voltage' && (
                        <View style={S.card}>
                            <Text style={S.cardTitle}>Low Voltage Sites</Text>
                            <STable
                                headers={['Site', 'Voltage (V)', 'Status', 'Action']}
                                rows={[
                                    ['Patna-North-09', '173', '🔴 Critical', 'Dispatch engineer'],
                                    ['Vadodara-South-04', '182', '🟡 Low', 'Remote battery check'],
                                    ['Ranchi-East-08', '190', '🔵 Attention', 'Monitor 24h'],
                                ]}
                            />
                        </View>
                    )}
                    ...
                    {activeTab === 'power' && (
                        <View style={S.card}>
                            <Text style={S.cardTitle}>SO vs Actual Power (Top Sites)</Text>
                            <STable
                                headers={['Site', 'SO (kW)', 'Actual (kW)', 'Diff']}
                                rows={[
                                    ['Delhi-Central', '60', '52', '-8'],
                                    ['Mumbai-West', '58', '63', '+5'],
                                    ['Kolkata-East', '56', '54', '-2'],
                                    ['Bengaluru-North', '62', '65', '+3'],
                                    ['Jaipur-City', '75', '63', '-12'],
                                ]}
                            />
                        </View>
                    )}
                    ...
                    {activeTab === 'powerfactor' && (
                        <View style={S.card}>
                            <Text style={S.cardTitle}>Power Factor Analysis</Text>
                            <STable
                                headers={['Site', 'PF', 'Status', 'Penalty']}
                                rows={[
                                    ['Delhi-Central-01', '0.82', '🟡 Warning', '₹465'],
                                    ['Kolkata-East-12', '0.91', '🟢 Normal', 'No penalty'],
                                    ['Mumbai-West-19', '0.88', '🔵 Attention', 'Monitoring'],
                                    ['Chennai-South-05', '0.80', '🔴 Critical', '₹610'],
                                ]}
                            />
                        </View>
                    )}
                    ...
                    {activeTab === 'sanctioned' && (
                        <View style={S.card}>
                            <Text style={S.cardTitle}>Load Optimization</Text>
                            <View style={S.warnBanner}>
                                <Icon name="alert-triangle" size={14} color="#f59e0b" />
                                <Text style={{ fontSize: 11, color: '#f59e0b', fontWeight: '600', flex: 1 }}>
                                    1 site within 5% of sanctioned load limit.
                                </Text>
                            </View>
                            <STable
                                headers={['Site', 'Sanctioned (kW)', 'Current (kW)', 'Status']}
                                rows={[
                                    ['Delhi-Central-01', '60', '58', '🟡 Approaching'],
                                    ['Jaipur-City-03', '75', '63', '🟢 Normal'],
                                    ['Chennai-South-05', '80', '72', '🔵 Attention'],
                                ]}
                            />
                        </View>
                    )}
                    ...
                    {activeTab === 'events' && (
                        <View>
                            {[
                                {
                                    title: "Cyclone 'Varun' Monitoring", type: 'Critical', color: C.danger,
                                    detail: "Emergency protocols active for 43 sites. Battery backup completed. 2 sites reported grid outages."
                                },
                                {
                                    title: "Rath Yatra Coverage", type: 'Festival', color: C.warning,
                                    detail: "Load increased 18% in covered regions. Additional generator deployed. No downtime reported."
                                },
                                {
                                    title: "VVIP Visit Monitoring", type: 'VVIP', color: '#3b82f6',
                                    detail: "High-availability enabled for 15 city sites. Status updates every 15 min. 1 minor voltage dip resolved."
                                },
                            ].map(ev => (
                                <View key={ev.title} style={[S.eventCard, { borderLeftColor: ev.color }]}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                        <Text style={{ fontSize: 13, fontWeight: '800', color: '#0f172a', flex: 1 }}>{ev.title}</Text>
                                        <View style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 12, backgroundColor: `${ev.color}15`, borderWidth: 1, borderColor: ev.color }}>
                                            <Text style={{ fontSize: 9, fontWeight: '800', color: ev.color }}>{ev.type}</Text>
                                        </View>
                                    </View>
                                    <Text style={{ fontSize: 11, color: '#64748b', lineHeight: 17 }}>{ev.detail}</Text>
                                </View>
                            ))}
                        </View>
                    )}
            ...
            <Modal visible={filterModal} transparent animationType="fade">
                <TouchableOpacity style={FM.overlay} onPress={() => setFilterModal(false)} activeOpacity={1}>
                    <View style={FM.box}>
                        <Text style={FM.title}>Filter Type</Text>
                        {FILTER_TYPES.map(f => (
                            <TouchableOpacity key={f.v}
                                style={[FM.opt, filterType === f.v && FM.optActive]}
                                onPress={() => applyFilter(f.v)}>
                                <Text style={[FM.optTxt, filterType === f.v && { color: C.primary, fontWeight: '800' }]}>{f.l}</Text>
                                {filterType === f.v && <Icon name="check" size={14} color={C.primary} />}
                            </TouchableOpacity>
                        ))}
                    </View>
                </TouchableOpacity>
            ...
            <Sidebar
                ...
                fullname={fullname}
                activeRoute="OptimizationReports"
                handleLogout={async () => {
