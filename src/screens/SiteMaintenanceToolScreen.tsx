...
import {
    ...
    RefreshControl, FlatList, TextInput, Share,
} from 'react-native';
import { api } from '../api';
...
export default function SiteMaintenanceToolScreen({ navigation, route }: any) {
    ...
    const handleShare = async () => {
        const rows = filtered.map(r =>
            `${r.site_id}\t${r.equipment_type}\t${r.installation_date || '—'}\t${r.status_label}`
        );
        const txt = [
        ...
        try { await Share.share({ message: txt, title: TAB_INFO[activeTab].label }); } catch (e) { }
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            <LinearGradient colors={['#012a4a', '#01497c']} style={styles.header}>
                ...
                <View style={{ alignItems: 'center', flex: 1 }}>
                    <Text style={styles.hTitle}>SITE MAINTENANCE TOOL</Text>
                    <Text style={styles.hSub}>{TAB_INFO[activeTab].label}</Text>
                </View>
...
const styles = StyleSheet.create({
    ...
    hBtn: { padding: 6 },
    hTitle: { color: '#fff', fontSize: 12, fontWeight: '800', letterSpacing: 1 },
