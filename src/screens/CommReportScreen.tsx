...
import {
    ...
    ActivityIndicator, SafeAreaView, ScrollView, RefreshControl, StatusBar, Alert, Platform
} from 'react-native';
import { api } from '../api';
...
export default function CommReportScreen({ navigation }: Props) {
    ...
    const fetchData = async (currentFilters = {}) => {
        ...
        try {
            ...
            rawSites.forEach(s => {
                const d = rawComm.find(c => c.imei === s.imei);
                if (!d) {
    ...
    const displayItems = useMemo(() => {
        if (!statusFilter) return currentCommGroup;
        return currentCommGroup.filter(s => 
            statusFilter === 'active' ? s.site_status === 'Active' : s.site_status !== 'Active'
        );
    }, [currentCommGroup, statusFilter]);
    ...
    return (
        <SafeAreaView style={styles.root}>
            ...
            {/* Standard Header matched to SiteStatusScreen */}
            <View style={styles.header}>
                <TouchableOpacity style={styles.backBtn} onPress={() => setSidebarVisible(true)} hitSlop={{ top: 15, bottom: 15, left: 15, right: 15 }}>
                    <Icon name="menu" size={24} color="#fff" />
                </TouchableOpacity>
                <Text style={styles.headerTitle}>Comm Report</Text>
                <View style={styles.headerIcons}>
                    <TouchableOpacity style={styles.iconBtn} onPress={handleExport} disabled={loading}>
                        <Icon name="download" size={22} color={loading ? "#ccc" : "#fff"} />
                    </TouchableOpacity>
            ...
            <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#1e3c72']} />}>
                ...
                {loading && !refreshing ? <ActivityIndicator size="large" color="#1e3c72" style={{ marginTop: 40 }} /> : (
                    <FlatList
                                ...
                                <View style={styles.card}>
                                    <TouchableOpacity style={styles.cardInfo} onPress={() => setExpandedImei(open ? null : item.imei)}>
                                        ...
                                        <View style={styles.cardEnd}>
                                            ...
                                            <Icon name={open ? "chevron-up" : "chevron-down"} size={20} color="#94a3b8" />
                                        </View>
                                    </TouchableOpacity>
                                    {open && (
                        ...
                        scrollEnabled={false}
                        ListEmptyComponent={<View style={styles.empty}><Icon name="info" size={40} color="#cbd5e1" /><Text style={styles.emptyText}>Nothing to show</Text></View>}
                    />
                )}
...
const Box = ({ label, count, color, icon, active, onPress }: any) => (
    <TouchableOpacity style={[styles.boxWrap, active && { borderColor: color, backgroundColor: '#fff', elevation: 4 }]} onPress={onPress}>
        <View style={[styles.boxIcoWrap, { backgroundColor: color }]}><Icon name={icon} size={11} color="#fff" /></View>
...
const Unit = ({ label, ok, l, m, t }: any) => (
    <View style={styles.unit}>
        <View style={styles.unitH}><Text style={styles.unitLabel}>{label}</Text><Text style={[styles.unitStat, { color: ok ? '#11998e' : '#ef4444' }]}>{l}</Text></View>
