    ...
    return (
      <View key={rowId} style={[styles.recordCard, idx % 2 === 0 && styles.recordCardAlt]}>
        ...
        <View style={styles.durationGrid}>
          ...
          <DurCell label="Total Running" val={runTotal} color="#2980b9" highlight />
        </View>

        {/* Sessions toggle */}
        {sessions.length > 0 && (
          <TouchableOpacity style={styles.sessionToggle} onPress={() => setExpandedRow(isExpanded ? null : rowId)}>
            <Icon name={isExpanded ? 'chevron-up' : 'chevron-down'} size={14} color="#01497C" />
              ...
              return (
                <View key={si} style={[styles.sessionItem, { borderLeftColor: sc.fg }]}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.sessionTime}>{fmtTime(s.start_time)} → {fmtTime(s.end_time)}</Text>
                  </View>
          ...
          <View style={styles.filterRow}>
            ...
            <View style={styles.filterField}>
              ...
              <TouchableOpacity style={styles.dateBtn} onPress={() => setShowPicker('start')}>
                <Icon name="calendar" size={14} color="#01497C" />
