...
export default function SiteVariationScreen({ navigation }: any) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      siteData.voltage_alarm_correlation.forEach(pt => {
        ...
        const alarms = (pt.active_alarms || []).join(' | ');
        csvContent += `"${pt.timestamp}","${pt.source_status}","${pt.running_status}","${parseFloat(pt.battery_voltage.toString()).toFixed(2)}","${mains}","${dg}","${alarms}"\n`;
      });

      // 2. Alarms
      csvContent += '\n"--- ALARM LOG ---"\n';
      csvContent += 'TIMESTAMP,ALARM NAME,STATUS,DURATION,VOLTS AT ALARM,DETAILS\n';
      siteData.detailed_alarms.forEach(al => {
