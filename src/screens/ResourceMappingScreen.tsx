...
import {
  ...
  RefreshControl, TextInput, Linking, Modal, Dimensions,
} from 'react-native';
        ...
        <View style={styles.modalSheet}>
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity onPress={onClose}>
              <Icon name="x" size={22} color="#1e3c72" />
            </TouchableOpacity>
          </View>
  ...
  return (
    <View style={styles.statsCard}>
      <Text style={styles.sectionTitle}>
        <Icon name="bar-chart-2" size={16} color="#1e3c72" />
    ...
    <View style={[styles.card, isActive ? styles.cardActive : styles.cardInactive]}>
      <View style={styles.cardHeader}>
        ...
        <View style={[styles.statusBadge, isActive ? styles.badgeActive : styles.badgeInactive]}>
          <Icon name={isActive ? 'wifi' : 'wifi-off'} size={11} color={isActive ? '#22c55e' : '#ef4444'} />
...
export default function ResourceMappingScreen({ navigation }: any) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (globalId) params.append('global_id', globalId);
      if (imei) params.append('imei', imei);
      if (statusFilter) params.append('status', statusFilter);
      params.append('export', 'excel');
      const url = `http://rms.shrotitele.com/api/site-summary/?${params.toString()}`;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <LinearGradient colors={['#1e3c72', '#2a5298']} style={styles.header}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.headerTitle}>Mapping of Resources</Text>
          <View style={styles.headerActions}>
            <TouchableOpacity onPress={handleExportExcel} style={styles.headerBtn}>
              <Icon name="download" size={20} color="#fff" />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setShowFilter(true)}
              style={[styles.headerBtn, hasActiveFilters && styles.filterActive]}
            >
        ...
        <View style={styles.kpiRow}>
          ...
          <View style={[styles.kpiBox, styles.kpiDivider]}>
            <Text style={[styles.kpiVal, { color: '#4ade80' }]}>{activeCount}</Text>
            <Text style={styles.kpiLab}>Active</Text>
          </View>
          <View style={styles.kpiBox}>
            <Text style={[styles.kpiVal, { color: '#f87171' }]}>{nonActiveCount}</Text>
            <Text style={styles.kpiLab}>Non-Active</Text>
          </View>
        </View>
        ...
        <View style={styles.tabBar}>
          ...
          >
            <Icon name="map" size={14} color={activeTab === 'map' ? '#1e3c72' : 'rgba(255,255,255,0.7)'} />
          ...
          >
            <Icon name="list" size={14} color={activeTab === 'list' ? '#1e3c72' : 'rgba(255,255,255,0.7)'} />
      ...
      {loading && !refreshing ? (
        ...
        >
          ...
          <View style={styles.mapCard}>
            <Text style={styles.sectionTitle}>
              <Icon name="map-pin" size={16} color="#1e3c72" />
        ...
        >
          {summary?.sites && summary.sites.length > 0 ? (
            summary.sites.map((item, i) => <SiteCard key={`${item.site_id}-${i}`} item={item} />)
          ) : (
            <View style={styles.empty}>
              <Icon name="server" size={60} color="#cbd5e1" />
...
const styles = StyleSheet.create({
  ...
  kpiRow: {
    ...
    borderRadius: 14, padding: 12, gap: 8, marginBottom: 14,
  },
  kpiBox: { flex: 1, alignItems: 'center' },
  kpiDivider: {
    borderLeftWidth: 1, borderRightWidth: 1,
    borderColor: 'rgba(255,255,255,0.25)', marginHorizontal: 4,
  },
  kpiVal: { fontSize: 20, fontWeight: '800', color: '#fff' },
  kpiLab: { fontSize: 10, color: 'rgba(255,255,255,0.75)', textTransform: 'uppercase', marginTop: 2 },
