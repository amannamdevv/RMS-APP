...
import {
    ...
    StatusBar, FlatList, TextInput, Alert, Share,
} from 'react-native';
import { api } from '../api';
    ...
    return (
        <View style={RF.card}>
            ...
            <TouchableOpacity style={RF.fileBtn} activeOpacity={0.8}
                ...
                <Icon name="paperclip" size={14} color="#01497c" />
            ...
            <TouchableOpacity style={[RF.submitBtn, submitting && { opacity: 0.6 }]}
                ...
                {submitting
                    ...
                    : (<><Icon name="send" size={14} color="#fff" /><Text style={RF.submitTxt}>Submit Ticket</Text></>)
                }
            </TouchableOpacity>
        </View>
    );
...
function MyTicketsTable({ filteredMy, onExport }: { filteredMy: any[], onExport: () => void }) {
    return (
...
export default function TTToolScreen({ navigation, route }: any) {
    ...
    const handleShare = async () => {
        ...
        if (activeTab === 'equipment') {
            txt = ['⚙ Equipment — ' + eqTab.toUpperCase(), '',
                'SITE ID\tTYPE\tINSTALL DATE\tSTATUS',
            ...eqData.map(r => `${r.site_id}\t${r.equipment_type}\t${r.installation_date || '—'}\t${r.status_label}`)
            ].join('\n');
        } else if (activeTab === 'repairs') {
            txt = ['🔧 Major Repairs', '',
                'TICKET\tSITE\tCATEGORY\tPRIORITY\tSTATUS\tDATE',
                ...repairs.map((t: any) => `${t.ticket_code || t.id}\t${t.site_id}\t${t.issue_category}\t${t.priority}\t${t.status}\t${t.raised_date}`)
            ].join('\n');
        } else if (activeTab === 'tickets' || activeTab === 'raise') {
            txt = ['🎫 My Tickets',
                `Open: ${counts.open}  In Progress: ${counts.progress}  Closed: ${counts.closed}`, '',
                'TT No\tSite ID\tCategory\tStatus\tRaised On',
                ...myTickets.map((t: any) => `${t.id}\t${t.site_id}\t${t.issue_category}\t${t.status}\t${t.raised_date}`)
            ].join('\n');
        }
        if (txt) { try { await Share.share({ message: txt, title: 'TT Tool Export' }); } catch (e) { } }
    };

    return (
        <SafeAreaView style={styles.container}>
            <StatusBar barStyle="light-content" backgroundColor="#01497c" />

            {/* Header */}
            <LinearGradient colors={['#01497c', '#013a63']} style={styles.header}>
                <TouchableOpacity onPress={() => setSidebarVisible(true)} style={styles.hBtn}>
                    <Icon name="menu" size={22} color="#fff" />
                </TouchableOpacity>
                <View style={{ alignItems: 'center', flex: 1 }}>
                    <Text style={styles.hTitle} numberOfLines={1}>{TAB_TITLES[activeTab]}</Text>
                    <Text style={styles.hSub}>Site Maintenance Tool</Text>
                </View>
                ...
                <>
                    ...
                    {activeTab === 'raise' && (
                        ...
                        >
                            ...
                            <MyTicketsTable filteredMy={filteredMy} onExport={handleShare} />
                        </ScrollView>
                    )}
...
const styles = StyleSheet.create({
    ...
    hSub: { color: 'rgba(255,255,255,0.7)', fontSize: 9, fontWeight: '600', marginTop: 2 },
